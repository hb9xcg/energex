...
void USI_TWI_Slave_Initialise( unsigned char TWI_ownAddress )
{
  ...
  DDR_USI  |=  (1<<PORT_USI_SCL);                                 // Set SCL as output Clock REC doesn´t like clock
                                                                    // stretching, thus output disabled
  ...
  switch (USI_TWI_Overflow_State)
  {
    ...
    case USI_SLAVE_CHECK_ADDRESS:
      if ((USIDR == 0) || (( USIDR>>1 ) == TWI_slaveAddress))
      {
        if ( USIDR & 0x01 )
		{
          USI_TWI_Overflow_State = USI_SLAVE_SEND_DATA; // read
		}
        else
        {
		  USI_TWI_Overflow_State = USI_SLAVE_REQUEST_DATA; // write
		}
		SET_USI_TO_SEND_ACK();
//		_delay_us(20); // Obviously SDA Ack needs some time to pass the optocouple?!?
      }
      else
      {
...
//      _delay_us(20); // Obviously SDA out needs some time to pass the optocouple?!?
	}
      break;
